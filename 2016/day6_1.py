...
#
# eedadn drvtee eandsr raavrd atevrs tsrnev sdttsa rasrtv nssdts ntnada svetve tesnvt vntsnd vrdear dvrsen enarar The most
# common character in the first column is e; in the second, a; in the third, s, and so on. Combining these characters
...
message = ""
for x in range(LENGTH):
    values[x] = sorted(values[x].items(), key=lambda x: x[1], reverse=True)
    # values[x] is now an array of tuples (character: frequency) sorted by frequency
    message += values[x][0][0]
